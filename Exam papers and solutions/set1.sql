...
-- 1. List the employees who got salary allowance between Rs.1000 to Rs.1500 in the year 
-- 2012. 
select emp.empcode, emp.empname, salary.allow, salary.salmonth
	from emp
    join salary
    on emp.empcode = salary.empcode
    where salary.allow between 1000 and 1500 and salary.salmonth between '2012-01-01' and '2012-12-31';


-- 2. List the oldest employee of the office
select emp.empcode, emp.empname, emp.birthdate, timestampdiff(year,emp.birthdate,curdate()) as year
	from emp 
    order by emp.birthdate asc
    limit 1;
    
-- 3. List the employees who have atleast two person working under him/her and the number 
-- of their subordinates. List the employee with highest number of subordinates first, next 
-- the person with next highest number of subordinates and so on. 
...
-- 4. List the departments in which average salary of employees is more than average salary of 
-- the company. 


-- 5. Create a procedure to count employees by gender from employee table. Use OUT 
-- keyword to store the count. Display the result of this procedure.

delimiter //
drop procedure if exists countGender //
create procedure countGender(in g char(1), out c int)
begin
	select count(empcode) into c 
		from emp
		where sex = g;
end//

-- set @count
-- call countGender('F', @count); //
-- select @count//

-- 6. Create a function to count number of employee born in a particular year. Execute this 
-- function by passing an year 1970

select year(emp.birthdate)
from emp;
